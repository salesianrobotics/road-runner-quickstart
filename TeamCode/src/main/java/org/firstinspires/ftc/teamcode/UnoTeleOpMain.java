...
@TeleOp(name = "UnoTeleOpMain", group = "Linear Opmode")
public class UnoTeleOpMain extends StandardBot {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            checkMotorCurrent("linearSlideMotor", linearSlide, OPTIMAL_CURRENT_TOLERANCE);

            if(gamepad1.left_bumper) // OPEN GRIPPER
            {
                gripperServo.setPosition(GRIPPER_OPENED_POSITION);

            }
            else if(gamepad1.right_bumper) // CLOSE GRIPPER
            {
                gripperServo.setPosition(GRIPPER_CLOSED_POSITION);
            }

            // Wrist Servo
            if(gamepad1.y) //wrist up
            {
                stdWristServo.setPosition(WRIST_UP_POSITION);
            }
            else if (gamepad1.a) // wrist down
            {
                stdGripperServo.setPosition(GRIPPER_MIDDLE_POSITION);

                wristServo.setPosition(WRIST_LOWER_GRAB_POSITION);
            }
            else if (gamepad1.x)
            {
